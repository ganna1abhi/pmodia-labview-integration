...
float start_freq; // Set start freq, < 100Khz
float incre_freq; // Set freq increment
int incre_num; // Set number of increments; < 511
float re;
float img;
float mag; 
float ph; 
...
void loop() {
 ...
 // if (Serial.available() > 0) {
    state = StateMachineState((int)Serial.parseInt()); 
  switch (state)
  {
    case (FREQUENCY):
      Serial.println("Freqency Sweep");
      FREQsweep();
      break;
...
void FREQsweep(){
  ...
  if (Serial.available() > 0){
    incre_freq = Serial.parseFloat();
  }
  Serial.println(incre_freq);
  // Increment by 1 kHz
  ...
  if (Serial.available() > 0){
    incre_num = Serial.parseInt();
  }
  Serial.println(incre_num);
  ...
  while (swp==1 && cnt < 50) {
    ...
    while (scheck != 0b00000010) {
     ...
     // Serial.print(PI);
      ...
      //Serial.println(ph); 
     // Serial.println(re);  Serial.println(img);
      Serial.println(re);
      //delay(10);
      Serial.println(img);
...
void Logging(){
 ...
 // }
  ...
  int lpar = int(start_freq)/int(incre_freq); 
  //Serial.println(lpar);
  ...
  writeData(CTRL_REG, 0x10);
  //delay(10);
  writeData(NUM_SCYCLES_R1, 128);
  //delay(10);
  writeData(NUM_SCYCLES_R2, 128);
  //delay(10);
  //  //status checking using polling - D2 needs to be high when done
   ...
   // Serial.println(swp);
    //Serial.println("logging");
     ...
     // Serial.print(PI);
      ...
      //Serial.println(ph); 
     // Serial.println(re);  Serial.println(img);
      Serial.println(re_un);
      //delay(10);
      Serial.println(img_un);
        writeData(CTRL_REG, 0x40);
       // Serial.print("actual mag: ");Serial.println(dev_freq);
        //delay(1000);
    // ClearBuffer();
    // WaitForData(); 
