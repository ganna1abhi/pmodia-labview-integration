...
IA::IA()
{
	...
	memset(StartFreqData, 0, sizeof(StartFreqData)); 
	memset(FrequencyIncData, 0 , sizeof(FrequencyIncData)); 
	memset(NumIncData, 0 , sizeof(NumIncData)); 
	memset(SettlingTimeData, 0 , sizeof(SettlingTimeData)); 
	...
	//Serial.println("data value in ai");
	//Serial.println(sizeof(IA_DATA.gf_value_100K));
}
...
double IA::getGF()
{
	...
	if(IA_DATA.range < 5){
		Serial.println("came in 20");
		IA_DATA.gf_addr = 20;}
	else{
		Serial.println("came in 100");
		IA_DATA.gf_addr = 100;}
	Serial.println("data value in getFG");
	Serial.println(sizeof(IA_DATA.gf_value_100K));
	EEPROMReadGF();
	...
	switch(IA_DATA.gf_addr)
	{
		...
		case 100: Serial.println(IA_DATA.gf_value_100K, 10);
				  gain_value = IA_DATA.gf_value_100K;
			break;
		default:
			break;
	}
